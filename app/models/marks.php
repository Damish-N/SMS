			...
			foreach ($marks_list as $key => $value) {
				if ($key !== 0) {
					...
					if ($result->rowCount() === 0) {
						...
						$result = $this->con->insert("stu-marks", ["student_id" => $st_id, "classroom_id" => $classroom_id, "exam_1_total" => 0, "exam_2_total" => 0, "exam_3_total" => 0, "exam_4_total" => 0, "exam_5_total" => 0, "exam_6_total" => 0]);
						if (!$result) {
							throw new PDOException();
						}
